"""
Script de prueba específico para la funcionalidad de carga masiva desde Excel
"""
...
def crear_archivo_excel_prueba():
    """Crear un archivo Excel de prueba para la carga masiva"""
    ...
    # Guardar archivo
    filename = 'test_carga_masiva.xlsx'
    wb.save(filename)
...
def probar_carga_masiva():
    ...
    print("="*60)
    print("PRUEBA DE CARGA MASIVA DESDE EXCEL")
    print("="*60)
    
    with app.app_context():
        try:
            # 1. Crear archivo Excel de prueba
            archivo_excel = crear_archivo_excel_prueba()
            ...
            # 3. Contar empleados y locadores antes de la carga
            print("\n3. Contando personal antes de la carga...")
            empleados_antes = Empleado.query.filter_by(empresa_id=empresa.id, activo=True).count()
            locadores_antes = Locador.query.filter_by(empresa_id=empresa.id, activo=True).count()
            ...
            empleados_despues = Empleado.query.filter_by(empresa_id=empresa.id, activo=True).count()
            locadores_despues = Locador.query.filter_by(empresa_id=empresa.id, activo=True).count()
            ...
            empleados_nuevos = Empleado.query.filter_by(empresa_id=empresa.id, activo=True).all()
            ...
            locadores_nuevos = Locador.query.filter_by(empresa_id=empresa.id, activo=True).all()
...
if __name__ == '__main__':
    print("INICIANDO PRUEBA DE CARGA MASIVA DESDE EXCEL...")
    if probar_carga_masiva():
